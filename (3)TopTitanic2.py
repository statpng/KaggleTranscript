...
def detect_outliers(df, n, features):
    ...
    # seelct observations containing more than 2 outliers
...
train.loc[Outliers_to_drop, ["Age", "SibSp", "Parch", "Fare"]]
...
cabin_levels = ["A", "B", "C", "D", "E", "F", "G", "T", "X"]
g = sns.countplot(dataset["Cabin"], order=cabin_levels)
g = sns.catplot(y="Survived", x="Cabin", data=dataset, kind="bar", order=cabin_levels)
...
pd.concat([dataset.Ticket, pd.DataFrame(Ticket)], axis=1)
pd.concat([dataset.Ticket, pd.Series(Ticket)], axis=1)
...
cv_res = pd.DataFrame({"CrossValMeans":cv_means,
                       ...
                       "Algorithm": ["SVC", "DecisionTree", "AdaBoost", "RandomFOrest", "ExtraTrees", "GradientBoosting", "MultilpleLayerPerceptron", "KNeighbors", "LogisitcRegression", "LinearDiscriminantAnalysis"]})
