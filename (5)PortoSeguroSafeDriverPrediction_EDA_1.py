...
# Besides that, it gives some tips and tricks to prepare you data for modeling.
...
# Okay, that's important information to get us started.
# Let's have a quick look at the first and last rows to confirm all of this.
...
#  - binary variables
#  - categorical variable sof which the category values are integers.
...
# Let's see if there are duplicated rows in the training data.
...
# So that's fine.
# Let's now investigate how many variable of each type we have.
...
# That's normal because missing values are already replaced by -1.
...
# We can also apply the describing method on the dataframe.
...
# Thanks to our meta file we can easily select the variables on which we wnat to compute the descriptive statistics.
...
#    We could apply scaling (e.g. StandardScaler), but it depends on the classifier we will want to use.
...
#  Perhaps some transofrmation (e.g. log) is already applied in order to anonymize the data?
...
# There are more strategies of course and MachineLearningMastery.com gives a nice overview.
...
# As we will create dummy variables later on, we need to check whether there are variables with many distinct values.
...
# EDIT: nickycan made an excellent remark on the fact that my first solution could lead to data leakage.
# He also pointed me to another kernel made by oliver which deals with that.
# I therefore replaced this part with the kernel of oliver.
# All credits go to him. It is so great what you can learn by participating in the Kaggle competitions :)
...
def target_encode(trn_series=None,
    ...
    min_smaples_leaf (int) : minimun samples to take category average into account
...
# Interval variables
# Checking the correlation between interval varibales.
# A heatmap is a good way to visualize the correlation between varibles.
...
# There are a strong correlation between the variables:
...
# We could use a piarplot to visualize the relationship between the variables.
...
# As the regression line shows, there is a linear relationship between theses variables.
...
sns.lmplot(x="ps_car_12", y="ps_car_14", data=s, hue="target", palette="Set2", scatter_kws={"alpha":0.3})


# ps_car_13 and ps_car_15

sns.lmplot(x="ps_car_13", y="ps_car_15", hue="target", data=s, palette="Set3", scatter_kws={"alpha":0.3})
...
# But as the number of correlated varibales is rather low, we will let the model do the heavy-lifting
...
# For instance, cateogry 2 is not twice the value of category 1.
# Therefore we can create dummy variables to deal with that.
# We drop the first dummy variable as this information can be derived from the other dummy variables generated for the categories of the original variabl.
...
# But if we would remove features with less than 1% variance, we would remove31 variables
...
# This repo contains code samples of hist book Python Machine Learning, which is an absolute must to read.
...
# With the get_support method we can then limit the number of variable sin the train data.
...
# And if you have questions, post a comment.
